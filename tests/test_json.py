...
def test_binding_names_and_when_expressions_must_not_be_empty() -> None:
    with pytest.raises(ValueError, match="binding names"):
        DagNode(slug="consume", plugin="consume", bindings={"": LiteralSource(value=True)})
...
def test_persisted_json_representation_comparison(
    left: object,
    right: object,
    equal: bool,
) -> None:
    assert json_representations_equal(freeze_json(left), freeze_json(right)) is equal
...
def test_node_and_plan_cardinality_limits_are_enforced() -> None:
    ...
    with pytest.raises(ValueError, match="too_long"):
        DagNode(slug="consume", plugin="consume", edges=edges)
    ...
    bindings = {
        f"parameter_{index}": LiteralSource(value=index) for index in range(MAX_NODE_BINDINGS + 1)
    }
    with pytest.raises(ValueError, match="too_long"):
        DagNode(slug="consume", plugin="consume", bindings=bindings)


def test_bounded_json_size_matches_canonical_serialisation() -> None:
    value = freeze_json(
        {
            "quoted": 'a"b\n',
            "unicode": "café 😀",
            "chunked": '"\n😀' * 2_000,
            "nested": [None, True, 1, 1.5, {"value": "x"}],
        }
    )
    expected = len(canonical_json_bytes(value))

    assert bounded_json_size(value, expected) == expected
    assert bounded_json_size(value, expected - 2) == expected - 1
...
def test_routing_models_support_deepcopy() -> None:
    output = OutputRef(id="output", fields={"nested": {"value": 1}})
    ...
    models = (
        output,
        EmittedOutput(id="output", fields=output.fields, data=resolved.data),
        resolved,
        TaskInputPlan(generation=1, parameters={"nested": {"value": 1}}),
        PlanningSnapshot(
            token="snapshot",
            generation=1,
            outputs_by_edge={"items": (output,)},
            source_dispositions={"items": NodeDisposition.LAUNCHED},
        ),
        TaskInputs(edges={"items": (resolved,)}, parameters={"value": 1}),
    )

    for model in models:
        copied = copy.deepcopy(model)
        assert copied is not model
        assert copied.model_dump(mode="json") == model.model_dump(mode="json")


def test_routing_models_generate_json_schema_with_object_values() -> None:
    for model_type in (
        DagNode,
        LiteralSource,
        OutputRef,
        ...
        json.dumps(model_type.model_json_schema())

    json_value = LiteralSource.model_json_schema()["$defs"]["JsonValue"]
    assert any(branch.get("type") == "object" for branch in json_value["anyOf"])
...
@pytest.mark.parametrize("invalid", [True, False, "1", 1.0])
def test_structural_integers_are_strict(invalid: object) -> None:
    constructors = (
        lambda: Selection(field="score", limit=invalid),
        lambda: PlannedEdgeInput(
            edge="records",
            source_index=invalid,
            role="main",
        ),
        lambda: TaskInputPlan(generation=invalid),
        lambda: TaskInputPlan.model_validate({"schema_version": invalid, "generation": 1}),
        lambda: PlanningSnapshot(
            token="snapshot",
            generation=invalid,
            outputs_by_edge={},
            source_dispositions={},
        ...
        lambda: TaskContext(run_id="run", node_index=invalid, generation=1),
        lambda: TaskContext(run_id="run", node_index=0, generation=invalid),
    )
...
def test_task_plan_enforces_reference_and_serialized_size_limits_on_load() -> None:
    ...
    with pytest.raises(ValueError, match="524288-byte limit"):
...
def test_output_models_share_the_routing_field_size_limit() -> None:
    fields = {"payload": "x" * MAX_OUTPUT_FIELDS_BYTES}

    for model_type in (OutputRef, EmittedOutput, ResolvedOutput):
        with pytest.raises(ValueError, match="65536-byte limit"):
            model_type(id="output", fields=fields)


def test_output_and_correlation_ids_accept_the_exact_limit_on_round_trip() -> None:
    output_id = "o" * MAX_OUTPUT_ID_LENGTH
    models = (
        OutputRef(id=output_id, correlation_id=output_id),
        EmittedOutput(id=output_id),
        ResolvedOutput(id=output_id),
        TaskInputPlan(
            generation=1,
            edges=(
                PlannedEdgeInput(
...
@pytest.mark.parametrize(
    "constructor",
    [
        lambda value: OutputRef(id=value),
        lambda value: OutputRef(id="output", correlation_id=value),
        lambda value: EmittedOutput(id=value),
        lambda value: ResolvedOutput(id=value),
        lambda value: PlannedEdgeInput(
            edge="records",
            source_index=0,
            role="main",
            output_ids=(value,),
        ),
        lambda value: TaskInputPlan(generation=1, correlation_id=value),
    ],
)
def test_output_and_correlation_ids_reject_values_over_the_limit(
    constructor: Callable[[str], object],
) -> None:
    with pytest.raises(ValueError, match=f"at most {MAX_OUTPUT_ID_LENGTH} characters"):
        constructor("o" * (MAX_OUTPUT_ID_LENGTH + 1))
...
def test_pipe_inputs_use_serialised_json_containers() -> None:
    value = LiteralSource(
        value={
    ...
    items = thaw_json(value["items"])
    record = thaw_json(value["record"])
