        ...
        snapshot=snapshot(
            {
                ...
                "right": (
                    shared.model_copy(
                        update={"fields": {"nested": {"enabled": True, "score": 1.0}}}
                    ),
                ),
            }
        ),
        ...
        snapshot=snapshot(
            {
                ...
                "right": (
                    shared.model_copy(
                        update={"fields": {"nested": {"score": 2, "enabled": True}}}
                    ),
                ),
            }
        ),
...
def test_correlation_default_resolution_and_emission() -> None:
    ...
    all_node = DagNode(slug="all", plugin="p")
    ...
    with pytest.raises(Exception, match="no correlation"):
    ...
    with pytest.raises(Exception, match="output ID"):
