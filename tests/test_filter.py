...
@pytest.mark.parametrize(
    ...
    [
        ...
        (FilterOperator.LT, "a", "b", True),
        (FilterOperator.LTE, "b", "b", True),
...
def test_exists_and_not_exists_treat_null_as_absent() -> None:
    ...
    exists = EdgeFilter(
        rules=(FilterRule(field="value", operator=FilterOperator.EXISTS),)
    )
    not_exists = EdgeFilter(
        rules=(FilterRule(field="value", operator=FilterOperator.NOT_EXISTS),)
    )
    assert [output.id for output in apply_filter(outputs, exists)] == ["present"]
...
def test_invalid_ordered_types_do_not_match() -> None:
    result = apply_filter(
        ...
        EdgeFilter(
            rules=(FilterRule(field="value", operator=FilterOperator.GT, value=0),)
        ),
    )
    assert result == ()
