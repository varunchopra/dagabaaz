...
def test_all_binding_sources_resolve_without_merging_namespaces() -> None:
    node = DagNode(
        ...
        bindings={
            ...
            "rank": ExpressionSource(expression="{records.metadata | first | json_get(rank)}"),
...
def test_missing_dynamic_bindings_remove_base_parameters_but_literal_null_remains() -> None:
    node = DagNode(
        slug="root",
        plugin="root",
        bindings={
            "edge": EdgeSource(edge="records", field="missing"),
            "runtime": RuntimeSource(key="missing"),
            "expression": ExpressionSource(expression="{records.missing}"),
            "literal": LiteralSource(value=None),
        },
    )
    ...
    assert parameters == {"literal": None}


def test_binding_condition_errors_are_not_treated_as_false() -> None:
    node = DagNode(
        slug="root",
        plugin="root",
        bindings={
            "value": LiteralSource(value="included", when="{input.value | required}"),
        },
    )

    with pytest.raises(ValueError, match="Required value is missing"):
        resolve_plan_bindings(node, {}, {})
