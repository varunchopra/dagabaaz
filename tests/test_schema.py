...
from dagabaaz.constants import MAX_RUN_INPUT_BYTES, InputMode
...
def test_merge_enforces_the_run_input_size_boundary() -> None:
    envelope_size = len(canonical_json_bytes({"payload": ""}))
    payload = "x" * (MAX_RUN_INPUT_BYTES - envelope_size)

    assert merge_run_input([], {}, {"payload": payload})["payload"] == payload
    with pytest.raises(ValueError, match="run input exceeds"):
        merge_run_input([], {}, {"payload": payload + "x"})
...
def test_input_schema_reports_empty_reference_parts(
    expression: str,
    error: str,
) -> None:
    node = DagNode(
    ...
    with pytest.raises(ExpressionError, match=error):
        get_pipeline_input_schema([node], lambda _name: None)
