...
class InputEdge(_FrozenModel):
    ...
    The name identifies the edge in plans, bindings and worker
    inputs. Filtering, selection, role and required status belong to the edge.
    """
...
class DagNode(BaseModel):
    ...
    Input and correlation modes are configured on the node.
    """
...
class OutputRef(_FrozenModel):
    ...
    def _check_size(self) -> OutputRef:
        size = len(canonical_json_bytes(self.fields))
        if size > MAX_OUTPUT_FIELDS_BYTES:
            raise ValueError(
                f"output fields are {size} bytes; maximum is {MAX_OUTPUT_FIELDS_BYTES}"
            )
        return self
...
class PlannedEdgeInput(_FrozenModel):
    ...
    def _check_output_ids(self) -> PlannedEdgeInput:
        if any(not output_id for output_id in self.output_ids):
            raise ValueError("planned output IDs must not be empty")
...
class TaskInputPlan(_FrozenModel):
    ...
    def _check_plan(self) -> TaskInputPlan:
        ...
        size = len(canonical_json_bytes(self.model_dump(mode="json")))
        if size > MAX_TASK_PLAN_BYTES:
            raise ValueError(f"task plan is {size} bytes; maximum is {MAX_TASK_PLAN_BYTES}")
        return self
